#include <stdio.h>
#include <stdlib.h>
#include <string.h>

char * optarg;
char * opt;
int argInd;
//from wikipedia
unsigned char s[256] = {  };
unsigned char si[256] = {  };
unsigned char m2[256] = {  };
unsigned char m3[256] = {  };
unsigned char m9[256] = {  };
unsigned char m11[256] = {  };
unsigned char m13[256] = {  };
unsigned char m14[256] = {  };
unsigned char g2[4][4] = { { 2, 3, 1, 1 }, { 1, 2, 3, 1 }, { 1, 1, 2, 3 }, { 3,
		1, 1, 2 } };
unsigned char g2i[4][4] = { { 14, 11, 13, 9 }, { 9, 14, 11, 13 }, { 13, 9, 14,
		11 }, { 11, 13, 9, 14 } };
unsigned char rcon[256] = {  };

int isoption(char * opt) {
	return opt[0] == '-';
}

//my version of getopt
int mygetopt(int argc, char * argv[], char * options) {
	int i, y;
	int res = -1;
	for (i = 0; i < argc; i++) {
		y = 0;
		//printf("cur argv is %s\n", argv[i]);
		if (argv[i] == '\0') {
			continue;
		}
		if (isoption(argv[i])) {
			//check if it matches an option
			while (options[y] != '\0') {
				//printf("matching for %c to %c", argv[i][1], options[y]);
				if (argv[i][1] == options[y]) {
					//found match
					res = options[y];
					//set the optarg
					optarg = argv[i + 1];
					argInd = i + 1;
					opt = argv[i];
					//return
					argv[i] = '\0';
					argv[i + 1] = '\0';
					return res;
				}
				y++;
			}

		}

	}
	return res;
}

void subBytes(unsigned char ** input) {
	int i, j;
	for (i = 0; i < 4; i++) {
		for (j = 0; j < 4; j++) {
			input[i][j] = s[(int) input[i][j]];
		}
	}
}

void inverseSubBytes(unsigned char ** input) {
	int i, j;
	for (i = 0; i < 4; i++) {
		for (j = 0; j < 4; j++) {
			input[i][j] = si[(int) input[i][j]];
		}
	}
}

void fillBuf(unsigned char * buf, unsigned char * in) {
	int i;
	for (i = 0; i < 4; i++) {
		buf[i] = in[i];
	}
}

void putRow(unsigned char * buf, unsigned char * in, int shift) {
	int i, val;
	for (i = 0; i < 4; i++) {
		val = (i + shift) % 4;
		in[i] = buf[val];
	}
}

void shiftRows(unsigned char ** input) {
	unsigned char buf[4];
	int i;
	//skip 1
	for (i = 1; i < 4; i++) {
		fillBuf(buf, input[i]);
		putRow(buf, input[i], i);
	}

}

void inversePutRow(unsigned char * buf, unsigned char * in, int shift) {
	int i, val;
	for (i = 0; i < 4; i++) {
		val = (i - shift);
		if (val < 0) {
			val += 4;
		}
		in[i] = buf[val];
	}
}

void inverseShiftRows(unsigned char ** input) {
	unsigned char buf[4];
	int i;
	//skip 1
	for (i = 1; i < 4; i++) {
		fillBuf(buf, input[i]);
		inversePutRow(buf, input[i], i);
	}
}

void fillCBuf(unsigned char * buf, unsigned char ** in, int column) {
	int i;
	for (i = 0; i < 4; i++) {
		buf[i] = in[i][column];
	}
}

int getMul(unsigned char num, int g2val) {
	//printf("num: %02x ", num);
	//printf("g2: %d ", g2val);
	switch (g2val) {
	case (1):
		return num;
	case (2):
		return m2[(int) num];
	case (3):
		return m3[(int) num];
	case (9):
		return m9[(int) num];
	case (11):
		return m11[(int) num];
	case (13):
		return m13[(int) num];
	case (14):
		return m14[(int) num];
	}
	return -1;
}

void doMultiply(unsigned char * buf, unsigned char * res) {
	int i;
	unsigned char v1, v2, v3, v4;
	for (i = 0; i < 4; i++) {
		v1 = getMul(buf[0], g2[i][0]);
		v2 = getMul(buf[1], g2[i][1]);
		v3 = getMul(buf[2], g2[i][2]);
		v4 = getMul(buf[3], g2[i][3]);
		//printf("vals: %02x %02x %02x %02x\n", v1, v2, v3, v4);
		res[i] = v1 ^ v2 ^ v3 ^ v4;
		//printf("res: %02x\n", res[i]);
	}
}

void doInverseMultiply(unsigned char * buf, unsigned char * res) {
	int i;
	unsigned char v1, v2, v3, v4;
	for (i = 0; i < 4; i++) {
		v1 = getMul(buf[0], g2i[i][0]);
		v2 = getMul(buf[1], g2i[i][1]);
		v3 = getMul(buf[2], g2i[i][2]);
		v4 = getMul(buf[3], g2i[i][3]);
		//printf("vals: %02x %02x %02x %02x\n", v1, v2, v3, v4);
		res[i] = v1 ^ v2 ^ v3 ^ v4;
		//printf("res: %02x\n", res[i]);
	}
}

void putColumn(unsigned char * res, unsigned char ** in, int column) {
	int i;
	for (i = 0; i < 4; i++) {
		in[i][column] = res[i];
	}
}

void printcb(unsigned char * b) {
	int i;
	for (i = 0; i < 4; i++) {
		printf("%02x ", b[i]);
	}
	printf("\n");
}

void mixColumns(unsigned char ** input) {
	unsigned char buf[4];
	unsigned char res[4];
	int i;
	for (i = 0; i < 4; i++) {
		fillCBuf(buf, input, i);
		//fillBuf(buf, input[i]);
		//printcb(buf);
		doMultiply(buf, res);
		//fillBuf(input[i], res);
		putColumn(res, input, i);
	}
}

void inverseMixColumns(unsigned char ** input) {
	unsigned char buf[4];
	unsigned char res[4];
	int i;
	for (i = 0; i < 4; i++) {
		fillCBuf(buf, input, i);
		//fillBuf(buf, input[i]);
		//printcb(buf);
		doInverseMultiply(buf, res);
		//fillBuf(input[i], res);
		putColumn(res, input, i);
	}

}

void addRoundKey(unsigned char ** input, unsigned char * roundKey) {
	int i, j, count;
	count = 0;
	for (i = 0; i < 4; i++) {
		for (j = 0; j < 4; j++) {
			input[j][i] ^= roundKey[count];
			count++;
		}
	}

}

void rotate(unsigned char * out, unsigned char * in, int shift) {
	unsigned char buf[4];
	fillBuf(buf, in);
	putRow(buf, out, shift);
}

void schedule(unsigned char * in, int roundNum) {
	//char c;
	int i;
	rotate(in, in, 1);
	for (i = 0; i < 4; i++) {
		in[i] = s[(int) in[i]];
	}
	in[0] ^= rcon[roundNum];
}

void expandKey(unsigned char * key, int key256) {
	unsigned char tbuf[4];
	int init = 16;
	int c = 16;
	int cSize = 176;
	int roundNum = 1;
	int i;
	if (key256) {
		c = 32;
		cSize = 240;
		init = 32;
	}
	while (c < cSize) {
		for (i = 0; i < 4; i++) {
			tbuf[i] = key[i + c - 4];
		}
		if (!(c % init)) {
			schedule(tbuf, roundNum++);
		}
		if ((key256) && (c % init == 16)) {
			for (i = 0; i < 4; i++) {
				tbuf[i] = s[(int) tbuf[i]];
			}
		}
		for (i = 0; i < 4; i++) {
			key[c] = key[c - init] ^ tbuf[i];
			c++;
		}
	}

}

void printX(char ** buf) {
	int i, j;
	for (i = 0; i < 4; i++) {
		printf("[");
		for (j = 0; j < 4; j++) {
			printf("%x ", buf[i][j]);
		}
		printf("]\n");
	}
}

void printC(char ** buf) {
	int i, j;
	for (i = 0; i < 4; i++) {
		printf("[");
		for (j = 0; j < 4; j++) {
			printf("%c ", buf[i][j]);
		}
		printf("]\n");
	}
}

void printT(unsigned char ** buf) {
	int i, j;
	for (j = 0; j < 4; j++) {
		for (i = 0; i < 4; i++) {
			printf("%02x", buf[i][j]);
		}
	}
	printf("\n");
}

//returns -1 if reached end of file, 0 if equal, 1 if padded
int fillText(unsigned char ** tbuf, FILE * f) {
	unsigned char buf[16];
	int i, j, count;
	int c;
	int eof = 0;
	for (i = 0; i < 16; i++) {
		c = fgetc(f);
		//if eof
		if (c == -1) {
			if(!i) {
				return -1;
			}
			buf[i] = '\0';
			i++;
			//pad buffer
			while (i < 16) {
				buf[i] = '\0';
				i++;
			}
			eof = 1;
			break;
		}
		buf[i] = c;
	}
	count = 0;
	for (j = 0; j < 4; j++) {
		for (i = 0; i < 4; i++) {
			tbuf[i][j] = buf[count];
			count++;
		}
	}

	return eof;
}

void writeText(unsigned char ** tbuf, FILE * out) {
	unsigned char buf[16];
	int i, j, count;
	count = 0;
	for (j = 0; j < 4; j++) {
		for (i = 0; i < 4; i++) {
			buf[count] = tbuf[i][j];
			count++;
		}
	}
	printcb(buf);
	fwrite(buf, 1, 16, out);
}

void aesDecrypt(unsigned char ** textbuf, unsigned char * roundKey, int k256) {
	int i;
	int end = 10;
	if (k256) {
		end = 14;
		roundKey += 224;
	} else {
		roundKey += 160;
	}
	//printT(textbuf);
	addRoundKey(textbuf, roundKey);
	//printT(textbuf);
	roundKey -= 16;
	inverseShiftRows(textbuf);
	//printT(textbuf);
	inverseSubBytes(textbuf);
	//printT(textbuf);
	for (i = 1; i < end; i++) {
		addRoundKey(textbuf, roundKey);
		//printT(textbuf);
		roundKey -= 16;
		inverseMixColumns(textbuf);
		//printT(textbuf);
		inverseShiftRows(textbuf);
		//printT(textbuf);
		inverseSubBytes(textbuf);
		//printT(textbuf);
	}
	addRoundKey(textbuf, roundKey);
	//printT(textbuf);
}

void aesAlg(unsigned char ** textbuf, unsigned char * roundKey, int k256) {
	int i;
	int end = 10;
	if (k256) {
		end = 14;
	}
	//printT(textbuf);
	addRoundKey(textbuf, roundKey);
	//printT(textbuf);
	roundKey += 16;
	for (i = 1; i < end; i++) {
		subBytes(textbuf);
		shiftRows(textbuf);
		//printT(textbuf);
		mixColumns(textbuf);
		//printT(textbuf);
		addRoundKey(textbuf, roundKey);
		//printT(textbuf);
		roundKey += 16;
	}
	subBytes(textbuf);
	//printT(textbuf);
	shiftRows(textbuf);
	//printT(textbuf);
	addRoundKey(textbuf, roundKey);
	//printT(textbuf);
}

void printKey(unsigned char * keybuf, int keysize) {
	int i = 0;
	while (i < keysize) {
		if (!(i % 16)) {
			printf("\n");
		}
		printf("%02x ", keybuf[i]);
		i++;
	}
}

int main(int argc, char * argv[]) {

	FILE * keyfile;
	FILE * inputfile;
	FILE * outputfile;
	int k256 = 0;
	int decrypt = 0;
	int keysize = 176;

	int i;
	int op;
	//code for handling options
	while ((op = mygetopt(argc, argv, "-")) != -1) {
		switch (opt[2]) {
		case 'k':
			if (opt[5] == 's') {
				//keysize
				k256 = strcmp("192", optarg);
			} else {
				//keyfile
				keyfile = fopen(optarg, "r");
			}
			break;
		case 'i':
			inputfile = fopen(optarg, "r");
			break;
		case 'o':
			outputfile = fopen(optarg, "w+");
			break;
		case 'm':
			decrypt = !strcmp("decrypt", optarg);
			break;

		}
	}
	if (k256) {
		keysize = 240;
	}
	unsigned char * keybuf = (unsigned char *) calloc(1, keysize + 1);
	int c;
	for (i = 0; i < 16; i++) {
		c = fgetc(keyfile);
		if (c == -1) {
			c = 0;
		}
		keybuf[i] = c;
	}
	fclose(keyfile);

	expandKey(keybuf, k256);
	printKey(keybuf, keysize);

	unsigned char ** textbuf = (unsigned char **) malloc(sizeof(char *) * 4);
	for (i = 0; i < 4; i++) {
		textbuf[i] = (unsigned char *) malloc(4);
	}

	int eof;
	while ((eof = fillText(textbuf, inputfile)) != -1) {
		if (!decrypt) {
			aesAlg(textbuf, keybuf, k256);
		} else {
			aesDecrypt(textbuf, keybuf, k256);
		}
		writeText(textbuf, outputfile);
	}

	fclose(inputfile);
	fclose(outputfile);
	return 0;
}

